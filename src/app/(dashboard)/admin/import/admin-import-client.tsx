    ...
    <div className="container max-w-4xl mx-auto p-10">
      ...
      <Card className="p-8 mb-6">
        <div className="space-y-6">
          ...
          {result && (
            <div
              className={`mt-6 p-6 rounded-lg ${result.success ? "bg-green-50 border border-green-200" : "bg-red-50 border border-red-200"}`}
            >
                  ...
                  {result.success && (
                    <div className="space-y-1.5 text-sm text-green-800">
                      <p className="flex items-center gap-1.5">
                        <Check className="h-3.5 w-3.5" /> Arquivos processados: {result.filesProcessed}/3
                      </p>
                      <p className="flex items-center gap-1.5">
                        <Check className="h-3.5 w-3.5" /> Contas criadas: {result.accountsCreated}
                      </p>
                      <p className="flex items-center gap-1.5">
                        <Check className="h-3.5 w-3.5" /> Transações importadas: {result.transactionsImported}
                      </p>
                      <p className="flex items-center gap-1.5">
                        <Check className="h-3.5 w-3.5" /> Duplicadas ignoradas: {result.transactionsSkipped}
                      </p>
                    </div>
                  )}

                  {result.error && <p className="text-sm text-red-800">{result.error}</p>}
      ...
      <Card className="p-8">
        <div className="space-y-6">
          ...
          {categorizationResult && (
            <div
              className={`mt-6 p-6 rounded-lg ${categorizationResult.success ? "bg-green-50 border border-green-200" : "bg-red-50 border border-red-200"}`}
            >
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <h3
                    className={`font-semibold mb-2 ${categorizationResult.success ? "text-green-900" : "text-red-900"}`}
                  >
                  ...
                  {categorizationResult.success && (
                    <div className="space-y-1.5 text-sm text-green-800">
                      <p className="flex items-center gap-1.5">
                        <Check className="h-3.5 w-3.5" /> Total de transações: {categorizationResult.total}
                      </p>
                      <p className="flex items-center gap-1.5">
                        <Check className="h-3.5 w-3.5" /> Categorizadas automaticamente: {categorizationResult.categorized}
                      </p>
                      <p className="flex items-center gap-1.5">
                        <Check className="h-3.5 w-3.5" /> Requerem revisão: {categorizationResult.needsReview}
                      </p>
                    </div>
                  )}

                  {categorizationResult.error && <p className="text-sm text-red-800">{categorizationResult.error}</p>}
                </div>
              </div>
            </div>
          )}
