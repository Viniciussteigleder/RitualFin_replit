...
export function CalendarClient({ 
  ...
  const calendarContent = (
    <Card className="h-full rounded-[2.5rem] border-border shadow-sm overflow-hidden bg-card flex flex-col">
      <CardContent className="p-0 flex flex-col flex-1">
        ...
        <div className="grid grid-cols-7 grid-rows-5 flex-1 min-h-[700px]">
                      ...
                      <div className="flex flex-col gap-1.5 mt-auto">
                        ...
                        {dayEvents.length > 0 && (
                            <div className="flex flex-col gap-1 border-t border-dashed border-border pt-1 mt-1">
                                {dayEvents.map(ev => (
                                    <div key={ev.id} className="text-[9px] font-medium text-blue-600 flex items-center gap-1 xl:max-w-[100px] 2xl:max-w-none">
  ...
  const sidePanelContent = (
    <Card className="h-full rounded-[2.5rem] border-border shadow-sm bg-card flex-1 flex flex-col">
      <CardContent className="p-8 flex-1 flex flex-col gap-6">
        ...
        {!selectedDay ? (
            ...
            <div className="flex flex-col gap-6 overflow-y-auto pr-2 flex-grow min-h-0">
                {/* Summary for Day */}
                        ...
                        {selectedTransactions.map(tx => (
                            <div key={tx.id} className="flex items-center justify-between p-3 bg-secondary/30 rounded-xl border border-transparent hover:border-border transition-[background-color,border-color,color,box-shadow,opacity] duration-150">
                                <div className="flex flex-col overflow-hidden mr-2">
                                    ...
                                    <span className="text-[10px] text-muted-foreground line-clamp-1">{tx.category1}</span>
                                </div>
                                <span className={cn("text-xs font-bold whitespace-nowrap", tx.amount < 0 ? "text-red-500" : "text-emerald-500")}>
                        ...
                        {selectedEvents.map(ev => (
                            <div key={ev.id} className="flex items-center justify-between p-3 bg-blue-500/5 rounded-xl border border-blue-500/10">
                                <div className="flex flex-col mr-2">
                                    <span className="text-sm font-bold text-blue-700 dark:text-blue-300 line-clamp-2">{ev.name}</span>
  ...
  if (!isDesktop) {
    return (
      <div className="grid grid-cols-1 gap-8 h-full items-start pb-8">
        {calendarContent}
  ...
  return (
    <div className="h-full w-full max-h-[1000px]">
      <ResizablePanelGroup direction="horizontal" className="h-full items-stretch gap-2">
        <ResizablePanel 
          defaultSize={70} 
          minSize={40} 
          className="h-full min-h-[700px] transition-all"
        >
          {calendarContent}
        </ResizablePanel>

        <ResizableHandle withHandle className="w-2 rounded-full mx-2 bg-transparent hover:bg-border/50 transition-colors" />
